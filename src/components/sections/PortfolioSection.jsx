...
export default function PortfolioSection() {
  ...
  const activeMedia = useMemo(() => (activeItem ? (mediaByItem[activeItem.id] || []) : []), [activeItem, mediaByItem]);
  ...
  const openModal = (id) => {
    setActiveId(id);
    // Bootstrap 5 modal: trigger via data-bs-target attribute in markup
    // We set activeId first; the button click opens modal.
  };
            ...
            return (
              <div className="col-12 col-md-6 col-lg-4" key={it.id}>
                <div className="card border-0 shadow-sm h-100">
                  {coverUrl ? (
                    <img src={coverUrl} alt={cover?.alt || it.title} className="card-img-top" style={{ height: 180, objectFit: "cover" }} />
                  ) : (
                  ...
                  <div className="card-body d-flex flex-column">
                    <div className="d-flex gap-2 align-items-center mb-1">
                      ...
                      {it.is_featured ? <span className="badge text-bg-warning ms-auto">Featured</span> : null}
                    </div>

                    {it.subtitle ? <div className="text-muted small mb-2">{it.subtitle}</div> : null}

                    {Array.isArray(it.tags) && it.tags.length ? (
                      <div className="d-flex flex-wrap gap-2 mb-3">
                        {asTextArr(it.tags).slice(0, 4).map((t, idx) => (
                          <span key={idx} className="badge text-bg-secondary">{t}</span>
                        ))}
                      </div>
                    ) : null}
            ...
            <div className="modal-content">
              <div className="modal-header">
                <div>
                  ...
                  {activeItem?.subtitle ? <div className="text-muted small">{activeItem.subtitle}</div> : null}
                </div>
                            ...
                            return (
                              <div key={m.id} className="border rounded overflow-hidden">
                                {m.media_type === "video" ? (
                                  ...
                                  <img src={url} alt={m.alt || activeItem.title} style={{ width: "100%" }} />
                                )}
                                {m.caption ? (
                      ...
                      {(activeItem.project_url || activeItem.repo_url) ? (
                        <div className="d-grid gap-2">
                          {activeItem.project_url ? (
                            <a className="btn btn-primary" href={activeItem.project_url} target="_blank" rel="noreferrer">
                              <i className="fa-solid fa-link me-2"></i>Live / Project Link
                            </a>
                          ) : null}
                          {activeItem.repo_url ? (
                            <a className="btn btn-outline-dark" href={activeItem.repo_url} target="_blank" rel="noreferrer">
